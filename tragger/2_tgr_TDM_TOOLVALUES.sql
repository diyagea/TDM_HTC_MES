GO
create trigger tgr_TDM_TOOLVALUES
on TMS.TDM_TOOLVALUES
    after insert,update --插入,更新触发
as
    --定义变量
	...
	if @count > 0
		BEGIN
			--更新已有记录
			update mes_tool set cuttingDiameter=@Dc, gaugeLength=@Xs, cuttingLength=@Ls, machiningDepth=@L4, updateTime=@exceTime where toolID=@compID and recMark = '!' ;
		END
	else if @count <= 0
		BEGIN
			--在中间表插入记录
